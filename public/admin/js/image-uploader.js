...
(function ($) {
    ...
    $.fn.imageUploader = function (options) {
        ...
        let createImg = function (src, id, preloaded, isVideo) {

            // Create the upladed image container
            let $container = $('<div>', {class: 'uploaded-image'});

            if( isVideo ) {
                let $img = $('<video controls><source src="' + src + '" type="video/mp4"></video>').appendTo($container);
            } else {
                // Create the img tag
        ...
        let setPreview = function ($container, files) {
            ...
            $(files).each(function (i, file) {
                ...
                // Set preview
                $uploadedContainer.append(createImg(URL.createObjectURL(file), dataTransfer.items.length - 1, false, isVideo), false);
